...
const PairProgramming = () => {
  return (
    <div>
      <h3 className='text-4xl font-bold w-7/12 m-auto mt-4'>Pair Programming</h3>
      <TeamList 
              ...
              <div className='w-4/12'>
                <ul className='w-full grid grid-cols-2 gap-6 mb-4'>
                  {sortedMembers.map(member => {
                    return (
                      <li 
                        className={`w-full ${member.name ? 'bg-green-400 rounded-lg ': ''}p-4 text-2xl font-semibold capitalize tracking-wide`}
                        key={uuidv4()}
                      >
                        {member.name}
                      </li>
